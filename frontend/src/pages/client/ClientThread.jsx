...
export default function ClientThread() {
  ...
  function load() {
    api.get(`/messages/mine/${id}`).then((d) => {
      setThread(d.message);
  ...
  async function send(e) {
    ...
    setSending(true);
    try {
      await api.post(`/messages/mine/${id}/reply`, { body: reply });
      setReply('');
      load();
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/client/messages" className="text-sm text-brand-700 hover:underline">← Back to messages</Link>
      ...
      <div ref={scrollRef} className="card max-h-[60vh] overflow-y-auto p-6">
        <Bubble
          who={thread.contactName || user.name}
          from="You"
          body={thread.body}
        ...
        {thread.replies?.map((r, i) => (
          <Bubble
            key={i}
            who={r.author?.name || (r.from === 'admin' ? 'Admin' : 'You')}
            from={r.from === 'admin' ? 'Admin' : 'You'}
            body={r.body}
            time={r.createdAt}
            tone={r.from === 'admin' ? 'admin' : 'client'}
          />
        ))}
      </div>
